...
from UpdateManager.DistSpecific.Ubuntu import UbuntuDist
from UpdateManager.DistSpecific import DistBase

class UbuntuCase(unittest.TestCase):
    def test0_implements_distbase(self):
        try:
            InterfaceValidator(DistBase, UbuntuDist).validate()
        except ValidationFailed, v_failed:
            self.fail(v_failed.message)

UbuntuSuite = loader.loadTestsFromTestCase(UbuntuCase)
